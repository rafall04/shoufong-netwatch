...
function MapContentInner() {
  ...
  return (
    <div 
      className="h-screen w-full relative bg-slate-50"
    ...
    >
      ...
      >
        ...
        <Panel position="top-left" className="bg-white/95 backdrop-blur-md border border-slate-200 p-1.5 rounded-2xl shadow-lg" style={{ marginTop: '128px' }}>
          ...
          >
            {isFullscreen ? (
              <Minimize2 className="w-4 h-4 text-slate-700" strokeWidth={1.5} />
            ) : (
              <Maximize2 className="w-4 h-4 text-slate-700" strokeWidth={1.5} />
            )}
          </button>
        </Panel>
        ...
        {session?.user?.role !== 'VIEWER' && (
          <Panel position="top-left" className="bg-white/95 backdrop-blur-md border border-slate-200 p-1.5 rounded-2xl shadow-lg" style={{ marginTop: '176px' }}>
            ...
            >
              <Link2 className="w-4 h-4 text-slate-700" strokeWidth={1.5} />
            </button>
          </Panel>
        )}
        ...
        <Panel position="top-right" className="bg-white/95 backdrop-blur-md border border-slate-200 rounded-2xl shadow-lg">
          <div className="p-3 space-y-2" style={{ fontVariantNumeric: 'tabular-nums' }}>
            ...
            <div className="flex items-center gap-3 text-sm">
              ...
              <span className="text-slate-600 font-medium">{data?.devices?.length || 0}</span>
            </div>
            
            {/* Last update & refresh button */}
            <div className="flex items-center justify-between gap-2 text-xs text-slate-500 border-t border-slate-200 pt-2">
              <div className="flex items-center gap-1.5">
                {isRefreshing ? (
                  <RefreshCw className="w-3.5 h-3.5 text-indigo-500 animate-spin" strokeWidth={1.5} />
                ) : (
              ...
              >
                <RefreshCw className={`w-3.5 h-3.5 text-slate-600 ${isRefreshing ? 'animate-spin' : ''}`} strokeWidth={1.5} />
              </button>
            </div>
          </div>
        </Panel>
        
        {/* Layout Tools - Floating Island */}
        {session?.user?.role !== 'VIEWER' && (
          <Panel position="top-center" className="bg-white/95 backdrop-blur-md border border-slate-200 rounded-2xl shadow-lg">
        ...
        {/* Legend Toggle - Floating Island */}
        <Panel position="bottom-left" className="bg-white/95 backdrop-blur-md border border-slate-200 p-1.5 rounded-2xl shadow-lg">
          <button
          ...
          >
            <Info className="w-4 h-4 text-slate-600" strokeWidth={1.5} />
          </button>
        </Panel>
        
        {/* Legend Content - Floating Island */}
        {showLegend && (
          <Panel position="bottom-left" className="bg-white/95 backdrop-blur-md border border-slate-200 rounded-2xl shadow-xl mb-12">
